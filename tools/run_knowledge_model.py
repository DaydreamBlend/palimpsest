...
def main():
    ...
    parser.add_argument('--max-output-tokens', type=int,
                        help='per-call local GLM output budget, up to the configured 256K ceiling')
    ...
    try:
        provider = (LocalGLMProvider(timeout_seconds=args.timeout_seconds or 1200,
                                     **({'max_output_tokens': args.max_output_tokens}
                                        if args.max_output_tokens is not None else {}))
                    if args.provider == 'local-glm' else
                    CodexProvider(timeout_seconds=args.timeout_seconds or 900))
