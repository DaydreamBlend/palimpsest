...
5. `python -X utf8 -B tools/validate_bundle.py --json`: exit1. [이번 결과](../output/t23-ui-realm/build-test-logs.zip#output/t04-input/document-validation.json)는 [직전 기준](../output/t23-ui-realm/build-test-logs.zip#output/t03-i-first-policy/document-validation.json)과 동일한12개 과거 raw/oracle Markdown 오류다. 새 오류는0이며 문서 검사를 앱 의미 검증으로 보고하지 않는다. 작업 중 문서 경로 조회1건과 원문 line 불일치 patch2건이 실패했고, patch는 부분 반영 없이 올바른 경로·문맥으로 재적용했다.
