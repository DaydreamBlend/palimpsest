...
1. usable source Information의 versioned retrieval projection
...
U11의 canonical source I는 원문 단위 전체를 보존한다. retrieval/model-context chunk·overlap·header/footer filter는 exact I/source range를 가진 별도 projection이며, 모델 교체나 context 제한 때문에 canonical I를 재작성·삭제하지 않는다.
...
5. D2I source-unit 처리 한 건마다 D2IRecord 하나를 만들고 결정적 처리 묶음은 `batch_id`로 표현한다. 과거 semantic proposal 기록은 보존한다.
6. canonical I는 구조·무결성 검사된 immutable source Information이며 `InformationRevision`을 두지 않는다. D2I의 임의 해석·가치 선별을 금지하며 의미 해석은 I2K부터 수행한다. 과거 semantic I는 원래 profile로 보존한다.
7. Information identity는 exact originating Data와 grounding에 종속된다. 서로 다른 D의 동일 semantic content를 하나의 Information으로 병합하지 않는다.
8. Information은 원문 내용·page/region·raw locator·parser profile/version·stable anchor와 전체 block coverage를 보존한다. 구조 검사는 의미의 진실이나 완전한 추출 충실성을 보장하지 않는다.
...
| `A2` | D2IRecord는 Information 처리 한 건당 하나이며 D2IOutput을 두지 않고 batch_id로 처리 묶음을 표현한다. U11 이후에는 결정적 source-unit 처리이며 이전 proposal 이력은 유지한다. |
...
| `A6` | semantic Generator와 Validator를 하나의 Operation pipeline에 두고 별도 KReviewRun을 두지 않는다. U11 D2I는 LLM 없이 source 구조 검사를 수행한다. |
...
| `A19` | Information은 그 자체가 immutable snapshot이며 InformationRevision을 두지 않는다. U11은 새 I를 source snapshot으로 정의하고 과거 semantic snapshot은 보존한다. |
