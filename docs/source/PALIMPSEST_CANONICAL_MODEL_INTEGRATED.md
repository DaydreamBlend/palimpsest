...
> 문서 상태: Current canonical architecture and terminology — integrated consistency revision 2026-09-09
>
> 언어: 한국어
>
...
6. `D2I`, `I2K`, `N2E`, `K2K`, `KQ2W`, `W2K`는 분석·제안·검증·편입을 수행하는 Operation이다.
7. `*Record`는 승인·기각·재사용·억제 fingerprint를 보존하는 영속 원장이다.
8. Candidate의 semantic payload는 Scriptorium의 임시 작업물이며 canonical 객체가 아니다.
9. 확정된 `Decision W`는 자동으로 W2K되어 `decision` KNode가 된다.
10. Knowledge propagation은 fingerprint/visited guard와 material-delta gate로 동일 계산을 반복하지 않으며, 더 이상의 material graph delta가 없는 fixed point까지 수렴한다. propagation의 크기·깊이·Record 수·token·cost cap은 정상 종료 조건으로 사용하지 않는다.
...
`Horreum`은 local artifact storage다.
...
- hash 검증을 통한 손상 탐지
- Bibliotheca가 가리키는 artifact를 읽을 수 있게 함

Horreum은 지식의 진실성, Information validation, Knowledge 승인 또는 검색 순위를 결정하지 않는다.

### 2.2 Bibliotheca

`Bibliotheca`는 canonical storage modules다.
...
Bibliotheca에는 승인되지 않은 I/K Candidate나 terminal rejected semantic payload를 넣지 않는다.

### 2.3 Scriptorium

`Scriptorium`은 knowledge compilation runtime이다.

주요 책임:

- D2I, I2K, N2E, K2K, KQ2W, W2K orchestration
...
초기 구현에서 Horreum, Bibliotheca, Scriptorium은 별도 네트워크 서비스일 필요가 없다. 하나의 애플리케이션과 하나의 PostgreSQL 안에서 directory/schema 책임으로 나눌 수 있다.
...
→ accepted/rejected similarity lookup
→ Validator
→ terminal disposition
→ atomic canonical commit or suppression
→ material-delta-gated downstream propagation until fixed point
```
...
| `W2KRecord` | confirmed Decision Wisdom | decision KNode |
...
공통 테이블을 사용하는 이유는 I2K와 K2K가 같은 KNode를 제안하더라도 전역 unique fingerprint guard를 적용하기 위해서다.
...
| `root_record_id` | 연쇄 propagation을 처음 시작한 Record ID다. 전체 causal tree와 fixed-point 수렴 과정을 묶는 기준이다. |
...
| `ordered_input_refs` | Generator와 Validator가 실제로 사용한 exact Information IDs와 K Revision IDs다. 순서가 의미에 영향을 줄 수 있으므로 canonical order와 FK rows를 보존한다. |
...
| `attempt_fingerprint` | Operation, trigger, ordered inputs, retrieval/profile version을 LLM 호출 전에 정규화한 digest다. 같은 분석 호출을 반복하지 않게 한다. |
...
| `context_fingerprint` | 후보 content, exact inputs, Validator가 실제 사용한 accepted/rejected neighbor refs, deterministic-check result/version, model·policy·schema version을 결합한 digest다. contextual/policy-scoped rejection suppression에 사용한다. |
...
pending 또는 needs-human 상태에서만 Scriptorium에 full semantic payload를 임시 보존한다.
...
| `temporary_embedding` | pending 후보를 accepted/rejected 근접 결과와 비교하기 위한 선택적 임시 vector다. canonical 검색 corpus에는 들어가지 않는다. |
...
| disposition | Candidate payload | Record | Bibliotheca |
|---|---|---|---|
...
| no_material_delta | 삭제 | 비교 결과 보존 | 변경 없음 |
...
terminal rejected payload를 canonical Bibliotheca에는 보존하지 않는 원칙은 유지한다. 다만 Generator/Validator regression, false-rejection 분석이 필요하면 Scriptorium의 비canonical bounded-retention audit store에 normalized payload와 validator finding을 일정 기간 보존할 수 있다.
...
검색 projection은 KEdgeRevision ID와 exact endpoint revision IDs를 결과에 포함한다.
...
I2K, N2E, K2K, W2K와 Information supersession/invalidation 기반 K 재검증은 서로 후속 작업을 만들 수 있다. Palimpsest는 이를 recursion이나 고정 크기 budget으로 자르지 않고, **material graph delta가 존재하는 frontier만 계속 전파해 fixed point에 수렴시키는 event propagation**으로 실행한다.
...
| 필드 | 설명 |
|---|---|
| `root_record_id` | 하나의 연쇄적 Knowledge compilation을 처음 시작한 Record다. 전체 causal tree와 fixed-point 수렴 과정을 묶는 기준이다. |
...
### 17.3 Fixed-point termination — propagation hard cap 없음

정상 종료 조건은 다음 하나다.

> **현재 propagation frontier에 downstream 재검증을 요구하는 material graph delta가 더 이상 존재하지 않는다.**
...
실제 변화가 10단계 뒤까지 이어지면 10단계까지, 100,000개의 객체에 material 영향이 있으면 원칙적으로 그 영향 frontier가 사라질 때까지 재검증한다. 반대로 바로 다음 객체에서 `no_material_delta`가 나오면 그 branch는 즉시 끝난다.
...
Query와 Memory를 canonical KGraph에 직접 삽입하지 않는다. KQ2W 동안만 임시 overlay를 만든다.
...
KQ2W retrieval mode는 epistemic boundary를 명시한다.
...
| `retrieval_mode` | `knowledge_only`, `knowledge_plus_evidence`, `evidence_only` 중 어떤 epistemic surface를 허용했는지 기록한다. |
...
| `confirmation_ref` | 클릭, 서명, 승인 event처럼 actor가 실제로 결정했음을 입증하는 confirmation evidence 참조다. |
...
5. downstream N2E/K2K outbox event insert
```
...
같은 Decision W가 retry되어도 decision KNode를 중복 생성하지 않는다. 반대로 서로 다른 `wisdom_id`의 Decision은 semantic payload가 같아도 별도 decision event이므로 기존 decision KNode로 content-based reuse하지 않는다.
...
active/superseded/expired 같은 `decision_status`는 semantic payload에 저장하지 않고 supersedes KEdge, lifecycle event, effective time을 이용해 계산하는 current projection으로 둔다.
...
| 현재 대화·일시 조건 | KQ2W Context snapshot |
| 장기 재사용할 외부 사실 | D→I→K 경로 |

Memory module이 존재하더라도 canonical truth store가 아니라 KQ2W에 임시 Context를 공급하는 runtime component다.
...
6. KGraph 변화가 과거 Query의 KQ2W를 자동 재실행하지 않는다.
